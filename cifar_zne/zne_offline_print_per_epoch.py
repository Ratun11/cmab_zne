# file: zne_offline_print_per_epoch.py
...
def main():
    ...
    ap.add_argument("--dumps_dir", default="zne_dumps_cifar")
    ap.add_argument("--split", default="test")  # CIFAR: use test
    ap.add_argument("--lams_zne", nargs="+", default=["1","3","5"])
    ap.add_argument("--lam_noiseless", type=int, default=0)
    ap.add_argument("--out_csv", default="zne_dumps_cifar/metrics_per_epoch.csv")
    args = ap.parse_args()
    ...
    for ep in epochs:
        ...
        for lam in lams_zne:
            ...
            lam_metrics[lam] = logits_metrics(q, W, b, labels)
        ...
        mse_noiseless_vs_lam1 = (loss_noiseless - lam_metrics[min(lams_zne)][0])**2
